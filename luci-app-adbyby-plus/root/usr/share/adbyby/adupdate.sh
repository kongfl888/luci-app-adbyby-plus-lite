...
touch /tmp/md5.json && uclient-fetch --no-check-certificate -T 10 -O /tmp/md5.json https://raw.githubusercontent.com/kongfl888/ad-rules/master/md5.json
...
if [ "$lazy_online"x != "$lazy_local"x -o "$video_online"x != "$video_local"x ]; then
    ...
    touch /tmp/lazy.txt && uclient-fetch --no-check-certificate -T 10 -O /tmp/lazy.txt https://raw.githubusercontent.com/kongfl888/ad-rules/master/lazy.txt
    touch /tmp/video.txt && uclient-fetch --no-check-certificate -T 10 -O /tmp/video.txt https://raw.githubusercontent.com/kongfl888/ad-rules/master/video.txt
